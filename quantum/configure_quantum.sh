...
TENANT_NAME="Project1"             # The tenant this network is created for
TENANT_NETWORK_NAME="project1-net" # The Quantum-internal network name
...
PROV_ROUTER_NAME="project1-router"
...
# Floating IP range
POOL_FLOATING_START="192.168.0.100"	# First public IP to be used for VMs
POOL_FLOATING_END="192.168.0.200"	# Last public IP to be used for VMs 
...
create_net() {
    ...
    tenant_net_id=$(get_id quantum net-create --tenant_id $tenant_id $tenant_network_name --provider:network_type gre --provider:segmentation_id 20)
...
create_ext_net() {
    ...
    ext_net_id=$(get_id quantum net-create $ext_net_name -- --router:external=True --provider:network_type gre --provider:segmentation_id 21)
...
echo ip addr flush dev $EXT_NET_BRIDGE
echo ip addr add $EXT_GW_IP/$EXT_NET_LEN dev $EXT_NET_BRIDGE
echo ip link set $EXT_NET_BRIDGE up
