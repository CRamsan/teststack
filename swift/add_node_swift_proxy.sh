...
SNODEIP=$(func_ask_user)

func_echo "The IP of the storage node is $SNODEIP"
...
swift-ring-builder account.builder add z1-$SNODEIP:6002/$SWIFTDEV 100
swift-ring-builder container.builder add z1-$SNODEIP:6001/$SWIFTDEV 100
swift-ring-builder object.builder add z1-$SNODEIP:6000/$SWIFTDEV 100
...
scp account.ring.gz "$TUSERNAME"@"$SNODEIP":~
scp container.ring.gz "$TUSERNAME"@"$SNODEIP":~
scp object.ring.gz "$TUSERNAME"@"$SNODEIP":~
